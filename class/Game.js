...
export default class extends EventEmitter {
	...
	constructor(canvas) {
		...
		this.on('settingsChange', settings => {
			let theme = document.querySelector('#game-theme'), href;
			if (theme && (href = theme.href.replace(/[^/]*(\.css)$/, `${this.settings.theme}$1`)) && href !== theme.href) {
	...
	async close(event) {
		this.#privateWritable && (await this.#privateWritable.write(this.scene.toString()),
